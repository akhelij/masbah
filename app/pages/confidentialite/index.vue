...
// Public privacy policy, adapted to the Masbah V2 model (peer-to-peer pool
// rental, cash on site, approximate location until acceptance, two-way reviews).
const { t } = useI18n()
const localePath = useLocalePath()
const localeHead = useLocaleHead({ seo: true })
const config = useRuntimeConfig()

// Sensible last-updated date for launch.
const lastUpdated = '2026-06-08'

useSeoMeta({
  title: () => `${t('legal.privacy.seoTitle')} — Masbah`,
  description: () => t('legal.privacy.intro'),
  ogTitle: () => `${t('legal.privacy.seoTitle')} — Masbah`,
  ogDescription: () => t('legal.privacy.intro'),
...
<template>
  <article class="legal">
    <header class="legal-head">
      ...
      <h1 class="t-h1">{{ t('legal.privacy.title') }}</h1>
      <p class="faint t-sm legal-date">{{ t('legal.lastUpdated', { date: lastUpdated }) }}</p>
    </header>

    <p class="t-bodyl legal-intro">{{ t('legal.privacy.intro') }}</p>

    <section v-for="(s, i) in sections" :key="i" class="legal-section">
      <h2 class="t-h3">{{ s.title }}</h2>
      <p class="t-body">{{ s.body }}</p>
    </section>

    <section class="legal-section legal-contact">
      <h2 class="t-h3">{{ t('legal.privacy.contactTitle') }}</h2>
      <p class="t-body">{{ t('legal.privacy.contactBody') }}</p>
    </section>
  </article>
</template>
